--Process with meter change loop added.
DECLARE
  --VARIABLES
                            ...
                            (SELECT MAX(STT.INSERT_DATE)
                              ...
                              WHERE STT.SUBDIVISION = P_SUBDIVISION_CODE
                                ...
                                AND NVL(STT.METER_SEQ, '1') =
                                    NVL(ST.METER_SEQ, '1'))
                      ORDER BY ST.CONS_NO, ST.METER_SEQ) LOOP -- G41_VALUES LOOP START
  
    -- LOOP FOR METER CHANGE CONSUMERS
    FOR METER_CHANGE_DATE IN (SELECT G.TRANS_DATE
                                FROM G11_HHD_FILE_DATA G
                               WHERE G.SUBDIVN = P_SUBDIVISION_CODE
                                 AND G.CONSUMER_NO = G41_VALUES.CONS_NO
                                 AND NVL(G.METER_SEQ, '1') =
                                     G41_VALUES.METER_SEQ
                                 AND G.PROCESS_FLAG IS NULL
                                 AND G.TRANS_DATE <=
                                     TO_DATE(G41_VALUES.BIL_DATE, 'YYYYMMDD')
                               ORDER BY G.TRANS_DATE) LOOP-- METER CHANGE LOOP START
        IF METER_CHANGE_DATE.TRANS_DATE =
           TO_DATE(G41_VALUES.BIL_DATE, 'YYYYMMDD') THEN
        
            SELECT *
              FROM G11_HHD_FILE_DATA MC
             WHERE MC.SUBDIVN = SUBSTR(P_CONSUMER_NO, 1, 3)
               AND MC.CONSUMER_NO = P_CONSUMER_NO
               AND MC.CONSUMER_NO = (SELECT C.CUSCODE
                                       FROM CUSTMASTER C
                                      WHERE C.SUBDIV_CODE = MC.SUBDIVN
                                        AND C.CUSCODE = MC.CONSUMER_NO
                                        AND C.CYCLE_NO=P_CYCLE_NUMBER)
               AND
                  
                   ((MC.TRANS_DATE <= P_BILL_DATE AND
                   NVL(MC.UPLOADED_SOURCE, ' ') = 'CRM') OR
                   NVL(MC.UPLOADED_SOURCE, ' ') = 'G11')
               AND MC.PROCESS_FLAG IS NULL;
        END IF;
    END LOOP; -- METER CHANGE LOOP END
...
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('PROCESS COMPLEATED UNSUCCESSFULLY');
